...
if($_POST['AjaxCheck'] === 'slugCheck'){
    if(isset($_POST['fieldValue'])) {
        ...
        $slugResult = $helper->checkSlugValidation($string,$tableType);
...
// if(isset($_POST['continueProduct'])){
//         // $fields =$_POST;
//         // $helper->storeProductInTable($fields);
//     // echo "<pre>";
//     // print_r($_POST);
//     // die;
// }

if($_POST['AjaxCheck'] === 'checkGroupSlug'){
    if(isset($_POST['groupId'])) {
        global $customadminpath;
        $path = $_SERVER['HTTP_HOST'] . '/' . $customadminpath;
       $groupSlug = Capsule::table('tblproductgroups')->where('id',$_POST['groupId'])->value('slug');

       echo $path . '/index.php?rp=/store/'.$groupSlug.'/';
        exit;
    }
}
