$(document).ready(function(){
    ...
    $(document).on('click', '#upgradeModule', function(event) {
            // event.preventDefault();
            // console.log('adkldkjlsk')
            $('#updateavailablemodal').modal('show');
    });
    $(document).on('change', '#inputGroup', function() {
       ...
       $.ajax({
        ...
        method: 'POST',
        data: {'AjaxCheck': 'checkGroupSlug','groupId':groupId},
        beforeSend: function(){
            $('#slugLoader').removeClass('hidden').show();
          },
        success: function(data) {
         $('#spanRoutePathProduct').html(data);
        },
        complete: function(){
            $('#slugLoader').addClass('hidden').show();
        }
    });
    });
    ...
    $(document).on('keyup', '#inputGroupName', function() {
        ...
        var tableType = 'productGroup';
             slugAjax(tableType);
            });

    $(document).on('keyup', '#inputProductName', function() {
        $('#slugLoader').removeClass('hidden').show();
        $('#slugOk').addClass('hidden').show();
        $('#slugInvalidError').addClass('hidden').show();
        groupName = $('#inputProductName').val();
        var tableType = 'product';
                slugAjax();
        });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(response['status'] == 'success'){
                            ...
                            swal("Failed", "You  Already Have Installed this Module", "error");
 ...
 //click sidebar    
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response['status'] != 'Active'){
                    swal("Failed", "Your Licence Key is "+ response['status'] + "!", "error");
                    return false;
                }
    ...
    $(document).on('click', '#continueProduct', function(event) {
        var productGroupName = $('#inputGroup').val();
        if(productGroupName == ""){
            event.preventDefault();
...
// adjust the size of Slug
    ...
    function slugAjax(tableType){
        $.ajax({
            url: '../modules/addons/agency_dashboard_pro/lib/ajaxFunction.php',
            method: 'POST',
            data: {'AjaxCheck': 'slugCheck', 'fieldValue': groupName,'tableType': tableType},
            dataType: 'json',
            success: function(data) {
            $('#slugLoader').hide();
            if (data.slugString) {
